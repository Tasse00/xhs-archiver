...
- **写入路径要确认一次**（`need_path`，排在采集者 ID 之后），之后常驻底部、任何状态下都可改，改动防抖 500ms 自动存
- **他人采过改为可接管**，写入路径默认改为 `collected/{日期}`（见下方决策表）
...
| 默认写入路径 `collected/{日期}`，不按采集者分目录 | 不要把采集者 ID 放回路径。归属记在指针文件名与 `note.json` 里，接管后目录名才不会失真 |
