...
小红书笔记归档 Chrome 扩展。**计划里的 13 个任务已全部实现，正在真实页面上做验收。** 已在实测中修掉的问题：笔记定位（见下）、正文夹带话题标签、缺最后编辑时间、采集结果与历史记录混淆、注入失败被误报成读不到全局变量、浏览页笔记列表滚不动。
...
- **写入路径要确认一次**（`need_path`，排在采集者 ID 之后），之后常驻底部只读展示；点「修改」进入独立设置页，保存时才持久化。设置页给四条快捷选项（`datasetPathPresets`）：`collected`、`collected/{今天}`、`{采集者}`、`{采集者}/{今天}`；**点了只填输入框，仍要再点保存**
...
- **竖向链子上的每一环都要显式 `min-height: 0`**：`.bw` → `.bw-main` → `.bw-list` →
  `.bw-table` → `.bw-scroll`。列表是虚拟化的，`.bw-scroll` 里撑着 `refs.length * 44px`
  的占位块，所以任何一环被内容撑开，滚动就落不到 `.bw-scroll` 身上。`.bw-table` 漏了
  这一行，实测（929px 视口 / 200 行）它被撑到 8859px，`.bw-scroll` 的 `clientHeight`
  等于 `scrollHeight` 等于 8800——没有溢出，没有滚动条，页脚被顶到视口外。
  **行数不足一屏时完全正常**，所以只有在真实仓库上才看得见。
- **这个 bug 单测抓不到**：项目的 Vitest 跑在 node 里，没有排版引擎，jsdom 的
  `clientHeight` 恒为 0。`tests/browser/layout-css.test.ts` 只能断言 CSS 里那行声明还在，
  真正的验证要靠浏览器量 `clientHeight` / `scrollHeight`。
...
- **`npm test` 之外必须自己跑一遍 `npx tsc --noEmit`**。Vitest 不做类型检查，`vite build`
  也不做，全仓库唯一跑 tsc 的地方是 Release workflow 的质量门。所以一个类型不过的 PR
  能带着全绿的测试合进 main，直到某次发布才炸——v0.3.0 之后就这么炸过一次。
