...
笔记写入路径为 `<root>/<dataset_path>/<note_id>/`。`dataset_path` 默认为 `collected/{YYYY-MM-DD}`，在侧边栏底部可就地编辑（例如改为 `collected/2026-08-03-outfit`），采集后记住上次使用的值作为下次默认。
...
**默认值要经使用者确认一次**（状态机里的 `need_path`，见第 8 节）。选完目录、设完 ID 之后单独出一屏，把完整的落盘路径 `<仓库>/<写入路径>/{笔记ID}/` 摆出来，确认后才进入正常流程。理由：路径决定整个仓库的组织方式，「有默认值」不等于「使用者知道数据会落在哪」。

确认之后写入路径常驻侧边栏底部，任何状态下都可改（没打开笔记时也在），改动防抖 500ms 后自动持久化。此前它只在「可采集 / 已采过」时才出现，导致配置完却没打开笔记的人根本看不到自己的数据会写到哪。
...
1. **只在笔记页上记。** `not_xhs`、`not_note`、未配置仓库/采集者这些状态一律不产生条目（`shouldLog`）。例外是 `panel_error`：侧边栏自己抛异常时无从判断当时在不在笔记页，丢掉它就丢掉了唯一线索。
