...
└── collected/2026-08-03/68a1b2c3d4e5f6/ 数据：{数据集路径}/{笔记ID}
    ├── note.json
...
更新、接管和迁移默认保留它；只有在侧边栏或浏览页主动修改或清空时才改变。
它不参与当前的搜索、筛选和排序。
...
插件发现某篇已被他人采集时，仍然可以更新它——这叫接管，在侧边栏里直接操作即可。
接管后这篇归到你名下：数据照写（原位或迁移到你的路径），你的指针写入，
对方的指针文件被删除。仓库里这篇始终只有一份。

因此 \`_index/68/68a1b2c3d4e5f6/\` 下永远只该有一个文件。出现多个，
说明发生了上一节的并发采集，按上一节处理。
