...
**目录被删掉时句柄不会失效，必须每周期显式探一次存在性**（`rootExists`，`missing_root` 状态）。使用者删掉/改名/移走仓库目录，或所在磁盘没挂上时，句柄仍在 IndexedDB 里，`queryPermission` 仍是 `granted`，只有真正读写才抛 `NotFoundError`。而 `store` 把 `NotFoundError` 一律解读成「没有这个条目」，于是**「仓库没了」与「仓库是空的」在读路径上完全同形**——不探就会一路走到「这篇还没人采过」，直到落盘那一刻才炸，而且炸在 `writeFile` 里逸出成未捕获的 rejection，界面毫无反应。

- 探法是**取一个条目就停**（`h.keys().next()`）：目录还在时空目录只是取不到条目，目录没了才抛。
- **排在权限之后**：没权限时探测本身抛 `NotAllowedError`。**排在采集者之前**：目录都没了，再往下每一步读盘都只会安静地返回空。
- 出口与 `need_permission` 相反：**只能重新选目录**，「恢复授权」救不了。但要同时给「重新检查」——最常见的两种成因（移走又移回、外置盘还没挂上）都是可逆的，不该逼人再走一遍目录选择器。
- `NotFoundError` 要单独识别（`isMissingError`）。遍历类操作（`listEntries`/`listDir`）不像读文件那样把它吞成「空」，会一路抛上来，所以顶层 catch 与 `doArchive` 两处都得接住。
- 浏览页同理：`PermissionGate` 拿到权限后先探一次，否则整个浏览页会安静地显示成「仓库是空的」。
...
**设置页给四条快捷选项**（`datasetPathPresets`）：`collected`、`collected/{今天}`、`{采集者}`、`{采集者}/{今天}`。日期取北京时区的当天（`todayBeijing()`），每次渲染重算——面板可能开着跨过零点。按日期与按采集者分目录几乎是全部的实际用法，手打 `collected/2026-08-07` 既慢又容易因为一个字符被校验拦下。

快捷选项**只填输入框，不直接保存**。路径决定整个仓库的组织方式，点一下就落盘等于把上面那条「确认一次」从流程里抽掉；仍要再点「保存修改」/「使用这个路径」。当前草稿命中哪一条，那一条就 `aria-pressed="true"`。

带采集者的两条是**顺手**，不是推荐——理由同上一段：多人协作的仓库里，按采集者分目录会让同一篇笔记的位置取决于谁先采到，接管之后目录名还会失真。个人独用时无妨。采集者 ID 为空或过不了 `isValidSegment`（旧版本可能存进过这种值）时，这两条不出现，否则就是给出一个点了必然保存失败的按钮。
...
1. **只在笔记页上记。** `not_xhs`、`not_note`、未配置仓库/采集者、待恢复授权、目录已不存在这些状态一律不产生条目（`shouldLog`）。例外是 `panel_error`：侧边栏自己抛异常时无从判断当时在不在笔记页，丢掉它就丢掉了唯一线索。
